...
不需要提前标注画框及标签等作为 Ground Truth 这种工作量巨大的人工手动操作， <br/>
直接上传测试图片，通过 HTTP API 调用算法模型得到推理后返回的 JSON 响应结果， <br/>
然后自动在图上绘制 label 标签、bounding box 画框, lines 连线, points 点等， <br/>
