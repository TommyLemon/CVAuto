...
不需要提前标注画框及标签等作为 Ground Truth 这种工作量巨大的人工手动操作， </a>
直接上传测试图片，通过 HTTP API 调用算法模型得到推理后返回的 JSON 响应结果， </a>
然后自动在图上绘制 label 标签、bounding box 画框, lines 连线, points 点等， </a>
自动断言画框是否正确，可点击边框来调整对错(√ 和 X)，一键上传可作为基准的断言结果， </a>
自动统计正确数、误报数、漏检数、召回率、精准率、F1 Score 等指标、自动对比前后效果。 </a>
...
**本项目是基于 机器学习 HTTP 接口工具 APIAuto 深度定制，原理和使用可参考 APIAuto 的文档及视频。**
